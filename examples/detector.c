...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        if(i%1000==0){
            ...
            char buff_mine[256];
            sprintf(buff_mine, "%s/%s_%d.backup", backup_directory, base, i);
            save_weights(net, buff_mine);
        }
        if(i % 10000 == 0 || (i < 20000 && i % 2000 == 0) || (i < 10000 && i % 1000 == 0) || (i < 1000 && i % 100 == 0)){
#ifdef GPU
            if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    list *plist = get_paths("data/images/test.txt");  // 変更 to chage the path for error
    char **paths = (char **)list_to_array(plist);
